...
public class FollowCamera : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (Math.Abs(deltaX) > 3){
            ...
            Vector3 newpos = Vector3.Lerp(transform.position, target, Time.deltaTime * smoothness* 5);	
            transform.position = newpos;

        }
        if (Math.Abs(deltaY) > 2){
            Vector3 target = transform.position + new Vector3 (0f, deltaY+1.75f, 0f);
            Vector3 newpos = Vector3.Lerp(transform.position, target, Time.deltaTime * smoothness);	
            transform.position = newpos;
        }
